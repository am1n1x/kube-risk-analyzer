...
- `severityChartInstance: null` — Chart.js instance for the doughnut chart (destroyed and re-created on each dashboard visit to prevent canvas overlap)
...
- Action methods: `init()`, `fetchData()`, `fetchScripts()`, `fetchDumps()`, `generateDump()`, `runOfflineScan()`, `runLiveScan()`, `runBas()`, `goToScan(scanId)`, `fetchScanDetails(scanId)`, `deleteScan(scanId)`, `exportScan(format)`, `saveRule()`, `deleteRule(ruleId)`, `saveScript()`, `deleteScript(scriptId)`, `toggleRule(rule)`, `downloadBackup()`, `serverBackup()`, `openRemediation(finding)`, `renderAttackGraph(finding)`, `renderSeverityChart()`, `getSeverityPct(scan, severity)`
- Computed getters: `filteredFindings`, `filteredRules`, `dashPods` (includes `lastScanId`), `dashTargets`, `filteredDashPods`, `filteredDashTargets`, `filteredScans`, `basNamespaces`, `basPodsForNs`, `liveScanPodsForNs`, `dumpPodsForNs`, `basDefenseRate`

**Frontend tabs:**
1. **Dashboard** — 4 stat boxes (Total Scans, Latest Scan Findings, Active Rules, **BAS Defense Rate**) + security telemetry section (60%/40% grid): **Security Posture Timeline** (pure CSS/HTML — last 5 scans, each row has date, target name, findings pill, stacked severity bar; clickable → `goToScan`) + **Latest Scan Severity** doughnut chart (Chart.js). Doughnut rendered via `renderSeverityChart()` on dashboard tab activation and after `fetchData()`; `getSeverityPct(scan, severity)` helper computes bar segment widths.
